...
\subsection{Insightful Caveats}
So we can reconstruct any latent \(h\) with a selection of a few key disposition vectors...what about the rest of the directions tied up in \(h\)? In the same vein of asking what we lose when we average local Jacobians to get \(J\) and \(\jlens\), we can ask about the epistemological consequences of very shrewdly dismissing \textit{most} of our \(\jlens\) vectors to get \(\jspace\) and \(\jspace(h)\). What information are we losing? A first thought might be: we only keep \(k\) directions to reconstruct all of \(h\), so we only keep \(\frac{k}{\dmodel}\) of the information. This would only be true if the rows of \(\jlens\) (from which we draw our \(k\)) were orthogonal and linearly independent. This is manifestly not the case.\footnote{For one thing, there are (typically) more rows (\(|V|\)) in \(\jlens\) than dimensions in each row \(\dmodel\). So even if every row could only be a basis vector, some would be duplicates, hence killing orthogonality.} Trained neural networks do not spread information uniformly through the spaces they're afforded. Training is a process of \textit{compressing}, \textit{abstracting}, and \textit{discovering} useful redundancies in the representation and apprehension of information. Because of this, the causal tendrils flowing from any given node within the model are braided and entangled, giving the model the power to make high-leverage decisions, but also blinding the path backwards through those decisions. \(\jlens\), as we know, was explicitly constructed by pulling derivatives backward, trying to reverse and untangle the causal course that determined the behavior we observed.

So if \(\jlens\) is highly redundant, then maybe \(k\) directions really can reconstruct almost all of the information in a given \(h\)...can it? At this point, we have to ask ourselves what we mean by \textit{information}. Information content is always in the eye of the beholder. In what ways can we measure the information in a latent \(h\) and compare it with that which is reconstructed in \(\jspace(h)\)? 

One neutral measure is simply the statistical variance. Of all the latents sampled in the process of computing \(\jlens\), what was their average information as denoted by variance? If they were all the same, variance would be null and therefore we could see that the information each one offers up on its own would be null as well. At each layer (we will omit subscripts denoting layer, because we can do this at any layer), we can define the variance as \(\mathscr{T} = \mathbb{E}_h\|h - \bar{h}\|^2\), where \(\bar{h} = \mathbb{E} [h]\)---the average squared length of the difference between any \(h\) and the average \(h\). This spread is a proxy for informational distinctness across the sampled activations of a layer---which, we'd hope, are distinct and decisive. Since we're interested in how good \(\jspace\) decompositions of latents are, we can see \(\mathscr{T}\) as a measure of \textit{the average information to be reconstructed}. How much information, then, do \(\jspace\) decompositions cover? The natural quantity has already been put to good use---the \textit{residual}. We can define an average measure of that which \(\jspace\) decompositions leave \textit{uncovered} as \(\mathscr{U} = \mathbb{E}_h\|h - \jspace(h)\|^2\). Finally, coupling these definitions, we can measure the average amount of variance covered by \(\jspace\) decompositions as \(\mathscr{C} = 1 - \frac{\mathscr{U}}{\mathscr{T}}\).\footnote{Note the asymmetry: \(\mathscr{U}\) is measured against \(h\) itself, while \(\mathscr{T}\) is measured about the mean \(\bar{h}\).}
% FOOTNOTE TO EXPAND (Ben) --- the thrust: this asymmetry makes \mathscr{C} a HARSHER test
% than a like-for-like one, and it cuts in our favor. \jspace(h) must reconstruct h outright,
% so some of its k directions are spent reproducing the large shared mean offset that every
% residual-stream latent carries --- work the \mathscr{T} baseline gets for free, since it is
% handed \bar{h} by construction. So 10--15% understates the non-trivial (off-mean) structure
% actually captured, and the concession we go on to make is more generous than it needs to be.
 \(\frac{\mathscr{U}}{\mathscr{T}}\) on its own tells us the fraction of what is \textit{uncovered} with respect to that which we want to cover. Subtracting this fraction tells us the fraction \textit{covered} of that which we want to cover. 

So what did Anthropic find for \(\mathscr{C}\)? When measured as we've described here, despite the success of the \(\jspace\) to be discussed in later sections, \(\mathscr{C}\) was typically only as high as \(10-15\%\). In typical LLMs, this is far higher than \(\frac{k}{\dmodel}\), but it is still a rather lower number than we might expect. The bulk of the variance of activations is unseen by the \(\jspace\) decomposition.

We said that information is in the eye of the beholder. What was the gaze that guided our constructed \(\jlens\)---the source from which we drew \(\jspace\)? How does it \textit{see} the raw data stored in a single latent? This is the crux: the computation of the \(\jlens\) only ever saw latents in terms of their downstream causal impact on logits. The power of \(\jlens\) is that it is \textit{not} a representation of static state, but instead disposition, informed by derivatives blaming \(h\). A hypothetical latent could be highly deviant from the mean (thus strongly contributing to \(\mathscr{T}\)) but be causally irrelevant, and therefore \textit{nothing} to the \(\jlens\). \(\jspace(h)\) doesn't reconstruct \(h\) through some geometrically neutral gaze---like that of \(\mathscr{T}\)---but instead does so in the basis of causal leverage itself. These top \(k\) directions picked from such a basis, yes, leave lots of variance on the table, but every one of them is, by construction, a direction of causal bearing on the network's outputs.
% PRECISION (Ben) --- worth a sentence or two here, and it strengthens the hand-off below.
% Split what is true BY CONSTRUCTION from what is EMPIRICAL:
%   (a) By construction: the dictionary is built of \jlens rows, i.e. logit-gradient
%       directions. So anything matching pursuit selects IS a leverage direction. Solid.
%   (b) NOT by construction: that the k selected are the ones carrying the MOST leverage.
%       The selection rule is <m_v, r> --- it ranks by what best rebuilds h, not by causal
%       magnitude. Leverage is the basis, not the ranking criterion.
%   (c) NOT by construction (the big one): that the DISCARDED remainder is causally
%       near-inert. That is the real claim of this section, and it is an empirical result,
%       not a theorem --- the remainder can still lie along lens rows we simply didn't pick.
% Suggest staking (c) plainly as a claim that must be EARNED, so the next paragraph's
% hand-off to \S\ref{sec:jfindings} (the 60%-vs-6% swaps) reads as a debt being paid
% rather than as a mere complement. Construction motivates; the interventions prove.

With this in mind, we see \(\jspace\) for what it is---the causal, dispositional profile of \(h\)---and what it isn't---a naive, generalist reconstruction of the static information frozen into the activations of any layer. The results explored in the following section complement the above discussed caveat, telling us that sheer \textit{volume} and true \textit{influence} can be separated. We already understand this notion of sparse leverage in the genome, for example. What \textit{makes} \textbf{J-vision} is the shedding of noise for causal signal, collapsing our focus to a crystallized, privileged subset---just as training did.
