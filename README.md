...
- **[`template<std::invocable<size_t> F> void ParForEach(size_t n, F f)`](src/TensorOps.hpp)**
...
Tensor<5, 4, 2> K;
auto out = ΣΠ<2>(W3, K);                          // contract last 2 dims of W3 (5,4) with (5,4) of K
...
static_assert(std::is_same_v<decltype(frob), Tensor<>>);  // scalar
