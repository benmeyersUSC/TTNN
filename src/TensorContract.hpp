...
auto BatchInnerContract(const Tensor<ADims...> &A,
    ...
    // TILE size can be chosen to match SIMD width (e.g., 8 for AVX2, 16 for AVX512)
    constexpr size_t TILE = 8;
    size_t num_full_tiles = ResultType::Size / TILE;
    size_t leftover = ResultType::Size % TILE;
    ...
    ParForEach(num_full_tiles, [&](const size_t t) {
        ...
        for (size_t tile_idx = 0; tile_idx < TILE; ++tile_idx) {
            ...
            size_t o = base + tile_idx;

            size_t batch = o / K::AB_Free_Size;
            size_t rem = o % K::AB_Free_Size;

            // this result index, o, corresponds to an element [freeA, freeB]
            // it is freeA * Free_B_Size + freeB
            // so:
            size_t free_a = rem / K::B_Free_Size; // how many full free B_Free tensors
            size_t free_b = rem % K::B_Free_Size;
    ...
    if (leftover > 0) {
        size_t base = num_full_tiles * TILE;
        std::array<float, TILE> acc{};
        acc.fill(init);
        for (size_t i = 0; i < leftover; ++i) {
            ...
            size_t free_a = rem / K::B_Free_Size;
            size_t free_b = rem % K::B_Free_Size;
