...
namespace TTTN {
    // @doc: struct NoLoss
    /** Sentinel type. Pass as `TrunkLoss` or `HeadBranch::Loss` to suppress loss computation and gradient contribution for that branch. */
    struct NoLoss {};
    ...
    class BranchTrainer {
        ...
        // @doc: template<typename TrunkLoss, size_t... Is> float BranchTrainer::compute_losses(const TrunkActivations &A, const TrunkOutputTensor &trunk_target, const HeadOutputs &head_targets, std::index_sequence<Is...>) const
        /** Accumulates scalar loss contributions from trunk and all heads */
        template<typename TrunkLoss, size_t... Is>
        float compute_losses(const TrunkActivations &A,
            ...
            ([&] {
                ...
                if constexpr (!std::is_same_v<HL, NoLoss>)
                    total += HL::Loss(
                        std::get<Is>(forward_heads(A, std::index_sequence_for<Heads...>{}))  // reuse head fwd
                            .template get<HeadI::Net::NumBlocks>(),
                        std::get<Is>(head_targets));
            }(), ...);
            return total;
        }

        // @doc: template<typename TrunkLoss, size_t Batch, size_t... Is> float BranchTrainer::compute_losses_batched(const TrunkNet::BatchedActivations<Batch> &A, const PrependBatch<Batch, TrunkOutputTensor>::type &trunk_target, const std::tuple<typename PrependBatch<Batch, typename Heads::Net::OutputTensor>::type...> &head_targets, std::index_sequence<Is...>) const
        /** Batched: accumulates scalar loss contributions from trunk and all heads across the batch */
        template<typename TrunkLoss, size_t Batch, size_t... Is>
        ...
        /** Recursive backward: chains `BackwardRange` through trunk segments, summing in precomputed per-head tap gradients */
        ...
        void backward_heads_batched(const TrunkNet::template BatchedActivations<Batch> &A,
                                    ...
                                    const std::tuple<typename PrependBatch<Batch, typename Heads::Net::InputTensor>::type...> &head_tap_grads) {
            constexpr size_t tapI = std::tuple_element_t<I, std::tuple<Heads...>>::Tap;
            ...
            [&]<size_t... Is>(std::index_sequence<Is...>) {
                ([&] {
                    ...
                    if constexpr (!std::is_same_v<HL, NoLoss>) {
                        ...
                        const auto  head_grad = HL::Grad(head_out, std::get<Is>(head_targets));
                        std::get<Is>(head_tap_grads) =
                            std::get<Is>(heads_).net.template BackwardRange<0, HeadI::Net::NumBlocks>(
                                head_A, head_grad);
